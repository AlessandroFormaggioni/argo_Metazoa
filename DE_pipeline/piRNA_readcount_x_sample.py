...
with open("piRNA_read_count.tab") as r:
...
for ll in range(20,31):
        print(str(ll))
        dict2={}
        with open("DaRe_"+str(ll)+"_overlap_10_table_fast.fasta") as r:
        ...
        for c in dict2.keys():
                ...
                if c in gp2:
                        new_row={"location":c+"_"+str(ll)+"_piRNA", "sp1":tot[0], "sp2":tot[1], "sp3":tot[2], "sp4":tot[3], "sp5":tot[4], "sp6":tot[5] }
                        df=df.append(new_row, ignore_index=True)
        with open("piRNA_doubles.tab","a+") as r:
                r.writelines('\t'.join([a+"_"+str(ll)+"_piRNA" for a in ciao]) + '\n' for ciao in gp )


df.to_csv("piRNA_readcount_x_sample.tab",sep="\t",index=False)
